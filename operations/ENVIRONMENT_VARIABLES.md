...
| `WINGMAN_LIVING_MAP_V1` | unset = **on** | Living Map is the default public surface. Set to `false` to roll back everyone to canvas Radar. Testers can also use `?radar=canvas` without changing the host flag. `?livingMap=1` is no longer required. |
