...
export function scoreCandidate(
  ...
  if (remaining !== undefined) {
    // Fresher availability (more TTL left / recent heartbeat) ranks higher
    if (remaining >= 60_000) {
      score += 0.12;
  ...
  const viewerLangs = input.viewerLanguages ?? [];
  const candLangs = c.languages ?? [];
  if (viewerLangs.length && candLangs.some((l) => viewerLangs.includes(l))) {
    score += 0.1;
    reasons.push("shared_language");
  }

  if (c.mood || c.intention) {
    score += 0.04;
  ...
  // Clamp; keep deterministic secondary key via userId in sort
  score = Math.max(0, Math.min(1, score));
  return { candidateId: c.userId, score: Math.round(score * 1000) / 1000, reasons };
}

export interface RankRadarResult {
  /** Same candidates as V1, reordered — public response uses this */
  ordered: EligibleCandidate[];
  /** Internal audit — never send scores to clients */
  audit: RankingAuditRecord;
}

/**
 * Rank eligible V1 candidates. Does not add/remove anyone.
 */
export function rankRadarCandidates(input: RankRadarInput): RankRadarResult {
  ...
  // Invariant: same multiset of userIds
...
/** Strip enrichment fields for public V1-compatible payload. */
